...
| Programmatically initiate Devin sessions | After deterministic evidence collection, the controller calls `POST /v3/organizations/{org_id}/sessions` with a bounded prompt, required structured-output schema, tags, selected knowledge, and an ACU limit. Publisher credentials remain in the controller. It persists the returned session ID before waiting. |
